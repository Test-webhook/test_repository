...
def get_tuple_for_closed_action(pr_data, action):
    closed_tuple = {ACTION: action, REPO: pr_data[BASE][REPO][FULL_NAME], NUMBER: pr_data[NUMBER],
                    ...
                    FILES: get_changed_files(pr_data[URL] + "/" + COMMITS + "/" + pr_data[MERGE_COMMIT_SHA]),
                    COMMITS: fetch_pr_commits(pr_data[URL] + "/" + COMMITS), COMMENTS: fetch_pr_comments(pr_data[URL]),
                    REVIEWERS: fetch_pr_reviewers(pr_data[URL] + "/" + REVIEWS),
